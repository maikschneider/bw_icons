...
function IconWizard($$anchor, $$props) {
  ...
  function fetchIcons() {
    ...
    new AjaxRequest(TYPO3.settings.ajaxUrls.icon_selection).post(body).then(async function(response) {
  ...
  var div_1 = child(div);
  var ul = child(div_1);
  each(ul, 21, () => get(tabs), index, ($$anchor2, tab, index2) => {
    var li = root_1();
    var a = child(li);
    a.__click = () => {
      set(activeTab, index2, true);
    ...
    template_effect(
      ($0) => {
        classes = set_class(a, 1, "nav-item nav-link", null, classes, $0);
        set_attribute(a, "href", `#${get(tab).title ?? ""}`);
        set_text(text2, get(tab).title);
      },
      [
        () => ({
          active: strict_equals(get(activeTab), index2)
        })
      ]
    );
    append($$anchor2, li);
  ...
  var node = child(span);
  html(node, () => $iconStore()["actions-filter"], false, false);
  reset(span);
  var input = sibling(span, 2);
  remove_input_defaults(input);
  input.__input = [on_input];
  var button = sibling(input, 2);
  button.__click = [on_click, filterQuery];
  ...
  each(div_7, 21, () => get(folders), index, ($$anchor2, folder) => {
    ...
    each(div_8, 21, () => get(folder).icons, index, ($$anchor3, $$item) => {
      ...
      var a_2 = root_6();
      let classes_1;
      a_2.__click = [on_click_1, selectedIcon, icon];
      var img = child(a_2);
      reset(a_2);
      template_effect(
        ($0) => {
          set_attribute(a_2, "href", `#${icon().title ?? ""}`);
          classes_1 = set_class(a_2, 1, "icon-grid-item svelte-nuufr", null, classes_1, $0);
          set_attribute(img, "src", icon().imgSrc);
          set_attribute(img, "alt", icon().title);
        },
        [
          () => ({
            active: strict_equals(get(selectedIcon), icon())
          })
        ]
      );
      append($$anchor3, a_2);
  ...
  reset(div_4);
  reset(div);
  bind_value(input, () => get(filterQuery), ($$value) => set(filterQuery, $$value));
  append($$anchor, div);
  var $$pop = pop({
    get itemFormElName() {
...
customElements.define("bw-icon-wizard", create_custom_element(
  IconWizard,
  {
    itemFormElName: {},
    itemFormElValue: {},
...
export {
  IconWizard as default
};
//# sourceMappingURL=IconWizard.js.map
